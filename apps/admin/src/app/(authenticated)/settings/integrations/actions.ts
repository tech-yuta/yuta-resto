...
export async function selectGoogleLocationAction(
  ...
  if (!selection.success) {
    redirect('/settings/integrations?google=invalid_location');
  }
  ...
  if (failure) redirect(`/settings/integrations?google=${failure}`);
  redirect('/settings/integrations?google=location_selected');
}
