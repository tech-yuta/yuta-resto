      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-5">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-yuta-line pb-5">
          <div>
            ...
            <h1 className="text-2xl font-black tracking-tight md:text-3xl">Commandes</h1>
            <p className="mt-1 text-sm text-yuta-ink/55">Suivi des commandes du service</p>
          </div>
          <Badge variant="neutral">{orderRows.length} commande(s)</Badge>
        </header>

        <nav className="flex gap-2 overflow-x-auto pb-1">
          {views.map((item) => (
            <Button key={item.value} asChild variant={item.value === selectedView ? 'primary' : 'secondary'} className="shrink-0">
              <Link href={`/orders?view=${item.value}`}>{item.label}</Link>
            </Button>
          ))}
        </nav>
        ...
        {orderRows.length === 0 ? (
          <Card className="grid min-h-80 place-items-center text-center">
            <div>
              ...
              <p className="mt-1 text-sm text-yuta-ink/55">Cette vue est vide pour le moment.</p>
            </div>
          </Card>
        ) : (
              ...
              {orderRows.map((order, index) => (
                <div key={order.id}>
                  <div className="grid gap-4 px-5 py-4 md:grid-cols-[1.2fr_1fr_0.8fr_0.8fr_0.8fr] md:items-center">
                    <div>
                      <p className="text-lg font-black md:text-base">{order.tableLabel}</p>
