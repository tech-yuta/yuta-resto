...
export async function GET(request: NextRequest) {
  ...
  if (!sessionToken) return redirectTo(request, '/login');

  try {
    const result = await authRepository.recoverSessionScope(sessionToken);
    if (result.type === 'NO_ESTABLISHMENT') {
      const response = redirectTo(request, '/access/no-establishment');
    ...
    if (result.type === 'SELECTION_REQUIRED') {
      const response = redirectTo(
        request,
        `/select-establishment?returnTo=${encodeURIComponent(returnTo)}`,
      );
      response.cookies.delete(BACKOFFICE_SESSION_COOKIE);
    ...
    const response = redirectTo(request, '/login?error=membership');
    response.cookies.delete(BACKOFFICE_SESSION_COOKIE);
    response.cookies.delete(BACKOFFICE_SELECTION_COOKIE);
    return response;
  }
